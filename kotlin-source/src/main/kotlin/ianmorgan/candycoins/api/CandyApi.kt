package ianmorgan.candycoins.api

import ianmorgan.candycoins.flow.*
 ...
 */
@Path("candy")
class CandyApi(val rpcOps: CordaRPCOps) {
    private val me = rpcOps.nodeInfo().legalIdentities.first().name

    companion object {
        private val logger: Logger = loggerFor<CandyApi>()
    }
    ...
    fun getBags(): List<StateAndRef<ContractState>> {
        return rpcOps.vaultQueryBy<CandyBagState>().states
    }
    ...
    fun issueIOU(@QueryParam(value = "chocolates") chocs: Int,
        ...
        try {
            val state = CandyBagState(chocs,gobs,beans,owner)
            // Start the IOUIssueFlow. We block and waits for the flow to return.
            val result = rpcOps.startTrackedFlow(::CandyBagGiveFlow, state).returnValue.get()
            // Return the response.
            return Response
