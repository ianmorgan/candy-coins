...
import ianmorgan.kandykoins.contract.IOUContract
...
class KandyBagGiveFlowTests {
    ...
    @Before
    fun setup() {
        mockNetwork = MockNetwork(listOf("ianmorgan.kandykoins"),
    ...
    fun flowReturnsCorrectlyFormedPartiallySignedTransaction() {
        ...
        val bag = KandyBagState(1,2,3, owner)
        val flow = KandyBagGiveFlow(bag)
        ...
        assert(ptx.tx.outputs.single().data is KandyBagState)
        val command = ptx.tx.commands.single()
        assert(command.value is KandyBagContract.Commands.Give)
